...
function [npp,ncpp] = h20_uncertainty_xxx(pdrykpa,vx,ekpa,frq,AMU);
...
S1 = MTX(:,3);     % LINE INTENSITIES AT 296K(?): %Nico [cm2*Hz] as seen on Janssen, table 2A.2 page 85 
...
WS = MTX(:,8)/1e3; % AIR-BROADENED WIDTH PARAMETERS AT ???K (WSELF -> WS) 
XS = MTX(:,9);     % T-EXPONENT OF AIR-BROADENING:
...
% Just a trial to be removed! To investigagte why we see some 3.5 K difference at first channel (22.2 GHz)
...
W3(1) = .00281;     % accounts for -4.00 K % it accounts for the most (the value 0.002665 was suggested by Payne et al., 2008)
