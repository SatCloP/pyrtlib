...
function [npp,ncpp,SUM] = h2o_uncertainty_r20sd(pdrykpa,vx,ekpa,frq,AMU);
...
% Perturb parameters ******************************************************
% Shape parameters ollowing Koshelev et al 2020
W0(1:2) = AMU.gamma0_a.value(1:2) / 1e3; % air-pressure width
W0S(1:2) = AMU.gamma0_w.value(1:2) / 1e3; % self-pressure width
X(1:2) = AMU.ng0_a.value(1:2); % temp exponent of air broadening
XS(1:2) = AMU.ng0_w.value(1:2); % temp exponent of self broadening
SH(1:2) = AMU.delta0_a.value(1:2) / 1e3; % AIR-BROADENED SHIFT PARAMETERS [MHz/mb -> GHz/mb]
SHS(1:2) = AMU.delta0_w.value(1:2) / 1e3; % SELF-BROADENED SHIFT PARAMETERS [MHz/mb -> GHz/mb]
XH(1:2) = AMU.nd0_a.value(1:2); % temp exponent of air shift
XHS(1:2) = AMU.nd0_w.value(1:2); % temp exponent of self shift
Aair(1:2) = AMU.A_a.value(1:2); % Frost parameter air-broadening shift [unitless]
Aself(1:2) = AMU.A_w.value(1:2); % Frost parameter self-broadening shift [unitless]
W2(1:2) = AMU.gamma2_a.value(1:2) / 1e3; % air-pressure width
W2S(1:2) = AMU.gamma2_w.value(1:2) / 1e3; % self-pressure width
D2AIR = AMU.delta2_a.value(2) / 1e3; % only for 183 GHz
D2SELF = AMU.delta2_w.value(2) / 1e3; % only for 183 GHz
...
%C      ADD RESONANCES
      ...
      TI = REFTLINE/T;
      TILN = log(TI);
      TI2 = exp(n_S*TILN); % == TI2 = TI^n_S; n_S = 2.5;
      SUM = 0.;
      for I = 1:NLINES
