...
function [awet,adry,aO2,aN2] = ClrAbs_ros19(p,tk,e,frq);
...
%c  Compute H2O and O2 absorption (ppm) and convert to dB/km (factor) and np/km. 
    [npp,ncpp] = h2o_rosen19_xxx(pdrykpa,v,ekpa,frq);
    awet(i) = (factor * (npp + ncpp)) * db2np;
    %[npp,ncpp] = o2n2_rosen17_xxx(pdrykpa,v,ekpa,frq); % NB to be replaced by O2 SD at 118 GHz, to be implement from o2abs_1-sd.f
    %adry(i) = (factor * (npp + ncpp)) * db2np;
    npp = o2abs_rosen18_xxx(pdrykpa,v,ekpa,frq); % NB to be replaced by O2 SD at 118 GHz, to implement from o2abs_1-sd.f
