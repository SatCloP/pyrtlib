...
function [npp,ncpp] = h20_ros2017_xxx(pdrykpa,vx,ekpa,frq);
...
W3 = MTX(:,5)/1e3; % AIR-BROADENED WIDTH PARAMETERS AT ???K (WAIR -> W3) [MHz/mb]
X  = MTX(:,6);     % T-EXPONENT OF AIR-BROADENING [unitless]
SR = MTX(:,7);     % RATIO OF SHIFT TO WIDTH [unitless]
WS = MTX(:,8)/1e3; % SELF-BROADENED WIDTH PARAMETERS AT ???K (WSELF -> WS) [MHz/mb]
XS = MTX(:,9);     % T-EXPONENT OF SELF-BROADENING [unitless]
