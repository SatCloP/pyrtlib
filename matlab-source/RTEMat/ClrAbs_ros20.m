...
% References:
%
% Rosenkranz, personal communication, 2019/03/18 - new coefficients, but without speed-dependance
...
function [awet,adry,aO2,aN2] = ClrAbs_ros20(p,tk,e,frq);
...
%c  Compute H2O and O2 absorption (ppm) and convert to dB/km (factor) and np/km. 
    [npp,ncpp] = h2o_rosen20_xxx(pdrykpa,v,ekpa,frq);
    awet(i) = (factor * (npp + ncpp)) * db2np; % wv only (lines and continuum)
    npp = o2abs_rosen19_xxx(pdrykpa,v,ekpa,frq); % (not sure it includes O2 SD at 118 GHz, i.e. o2abs_1-sd.f)
    aO2(i) = (factor * npp) * db2np; % oxygen only (lines and continuum)
% C add N2 term
    aN2(i) = ABSN2_ros18(tk(i),pdrykpa*10,frq); % ABSN2 takes in input DRY AIR PRESSURE (MB) and outputs NEPER/KM
